...
class CustomPdfGenerator extends TCPDF
{
    public function Header()
    {
        ...
        $this->Cell(0, 15, '', 0, false, 0, false, 'C', 0, '', 0, false, 'M', 'M');
        $this->Ln();
        $this->Cell(0, 15, 'Katie Falk', 0, false, 'R', 0, '', 0, false, 'M', 'M');
    }
    public function Footer()
    {
        $this->SetY(-15);
        $this->SetFont('helvetica', 'I', 15);
        $this->Cell(0,10, 'Thank you for your business!', 0, false, 'C', '', 0, false, 'T', 'M');
    }
    public function printTable($header, $data)
    {
        ...
        foreach($data as $row)
        {
            ...
            $this->Cell($w[2], 6, number_format($row[2]), 0, 'R', $fill);
            $this->Cell($w[3], 6, number_format($row[3]), 0, 'R', $fill);
            $this->Ln();
            $fill =! $fill;
            $total += $row[3];
        }
